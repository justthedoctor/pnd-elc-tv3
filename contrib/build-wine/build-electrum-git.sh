...
pushd $WINEPREFIX/drive_c/electrum-pnd
...
rm -rf "$WINEPREFIX/drive_c/electrum-pnd/electrum_pnd/locale/"
for i in ./locale/*; do
    dir="$WINEPREFIX/drive_c/electrum-pnd/electrum_pnd/$i/LC_MESSAGES"
    mkdir -p $dir
...
cd dist
mv electrum-pnd-setup.exe $NAME_ROOT-$VERSION-setup.exe
cd ..
